...
const currentSnapshot=async(force=false)=>live?decorateRealResult(await live.read(force),realOverlays):snapshot();
const setStationNumber=(current,set)=>{const direct=Number(set?.stationNumber);if(set?.stationNumber!=null&&Number.isInteger(direct))return direct;const station=current.state.stations.find(candidate=>candidate.id===set?.stationId);return Number(station?.number);};
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(url.pathname==='/api/state'&&req.method==='GET')return json(res,200,await currentSnapshot(url.searchParams.get('force')==='1'));
      ...
      if(user?.user.provider==='start.gg'){
        ...
        return await enqueue(async()=>{
          ...
          if(action.type==='assign'){
            ...
            if(current.state.sets.some(candidate=>candidate.id!==set.id&&['called','playing'].includes(candidate.status)&&setStationNumber(current,candidate)===Number(station.number)))return json(res,409,{message:`La estación ${station.number} ya está ocupada.`});
    ...
    const publicFiles={'/':'index.html','/index.html':'index.html','/previous_index.html':'previous_index.html','/public-app.js':'public-app.js','/public-styles.css':'public-styles.css','/app.js':'app.js','/core.js':'core.js','/transport.js':'transport.js','/config.js':'config.js','/styles.css':'styles.css','/favicon.svg':'favicon.svg','/logo.jpeg':'logo.jpeg','/logo1.jpeg':'logo1.jpeg','/logo2.jpeg':'logo2.jpeg','/logo3.jpeg':'logo3.jpeg'};
    const file=publicFiles[url.pathname];if(!file)return json(res,404,{message:'Página no encontrada.'});
    if(file==='config.js'){res.writeHead(200,{'Content-Type':'text/javascript; charset=utf-8','Cache-Control':'no-store'});return res.end(`export const CONFIG = ${JSON.stringify({name:'Salamandra Queue',mode:'node',backend:'node',apiBase:'/api',refreshMs:15000,fullReloadMs:120000,oauthEnabled:oauth.enabled,poolSchedules:{A:{date:'26 de septiembre de 2026',time:'10:00–10:15 a. m.'},B:{date:'26 de septiembre de 2026',time:'12:00–12:15 p. m.'}}})};`);}
